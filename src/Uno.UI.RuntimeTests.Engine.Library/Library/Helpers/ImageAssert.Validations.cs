...
partial class ImageAssert
{
	...
	private static bool Validate(ExpectedPixels expectation, TestBitmap actualBitmap, double expectedToActualScale, StringBuilder report)
	{
	...
	{
		var expectedColor = expectation.Values[(int)pixel.Y, (int)pixel.X];
	...
	private static Rect Normalize(Rect rect, Size size)
		=> new(
